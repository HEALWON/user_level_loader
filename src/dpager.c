    ...
    for (int i = 0; i < ehdr->e_phnum; i++) {
        ...
        if (phdr->p_type == PT_LOAD) {
            ...
            if ((addr >= phdr->p_vaddr) && (addr < (phdr->p_vaddr + phdr->p_memsz))) {
                ...
                uint64_t memsz = (PGSIZE < filesz)? 0 : PGSIZE - filesz;

                uint64_t offset = ALIGN_LOW(phdr->p_offset) + addr_align - vaddr_align;

                if (ALIGN_HIGH(phdr->p_vaddr + phdr->p_filesz) < addr) {
                    flags |= MAP_ANON;
                    fd = -1;
                    offset = 0;
                    printf("anon ");
